...
function drawViz(data) {
  let data1 = data.fields.dimID;
  let data2 = data.tables.DEFAULT;
  ...
  const styleData = {
    ...
    legendColor1: data.style.legend_color1.value.color || data.style.legend_color1.defaultValue,
    legendColor2: data.style.legend_color2.value.color || data.style.legend_color2.defaultValue,
    legendColor3: data.style.legend_color3.value.color || data.style.legend_color3.defaultValue,
    legendHeight: data.style.legendheight.value || data.style.legendheight.defaultValue,
    legendWidth: data.style.legendwidth.value || data.style.legendwidth.defaultValue,
    legendborderRadius: data.style.legendborderradius.value || data.style.legendborderradius.defaultValue,
    legendSpacing: data.style.legendspacing.value || data.style.legendspacing.defaultValue,
  };

  if (document.querySelector("div")) {
    let oldTable = document.querySelector("div");
    oldTable.parentNode.removeChild(oldTable);
  }

  let tbl = document.createElement("div");
  tbl.setAttribute("class", "main_container");

  let tblElement = document.createElement("div");
  tblElement.setAttribute("class", "tbl_style");

  for (let i = 0; i < data1.length; i++) {
    let cell = document.createElement("div");
    let div = document.createElement("div");
    cell.setAttribute("class", "label_style");
    cell.setAttribute("style", `height: ${styleData.legendSpacing}px;`)
    div.setAttribute("class", "cell_style");
    let innerDiv = document.createElement("div");
    innerDiv.setAttribute("class", "tbl_style_sub");
    innerDiv.setAttribute("style", `font-family: ${styleData.textFont}; font-size: ${styleData.textSize}px; color: ${styleData.textColor}`);
    let cellValue = document.createTextNode(data1[i].name);
    let cellValue2;

    if (data2.length > 0) {
      // cellValue2 = document.createTextNode(data2[0].dimID[i]);
      // if (data2[0].dimID[i] > 0) {
      //   div.setAttribute("style", `background-color: ${styleData.legendColor1}; color: ${styleData.legendColor1}; height: ${styleData.legendHeight}px; width: ${styleData.legendWidth}px; border-radius: ${styleData.legendborderRadius}px`);
      // } else if (data2[0].dimID[i] == 0) {
      //   div.setAttribute("style", `background-color: ${styleData.legendColor2}; color: ${styleData.legendColor2}; height: ${styleData.legendHeight}px; width: ${styleData.legendWidth}px; border-radius: ${styleData.legendborderRadius}px`);
      // } else {
      //   div.setAttribute("style", `background-color: ${styleData.legendColor3}; color: ${styleData.legendColor3}; height: ${styleData.legendHeight}px; width: ${styleData.legendWidth}px; border-radius: ${styleData.legendborderRadius}px`);
      // }
      for(let j=0;)
    } else {
      cellValue2 = document.createTextNode("");
    }

    // height: ${styleData.legendSize}px; width: ${styleData.legendSize}px;

    cell.appendChild(cellValue);
    div.appendChild(cellValue2);
    innerDiv.appendChild(cell);
    innerDiv.appendChild(div);
    tblElement.appendChild(innerDiv);
  }
  tbl.appendChild(tblElement);
